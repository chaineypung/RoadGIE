...
class FlexiblePromptEmbed(SuperFlexiblePrompt):
    ...
    def update_embed(self, prev_input: Dict, new_input: Dict = None, mask_pred: Optional[torch.Tensor] = None, new_prompt=True):
        ...
        if not self.sam:
            ...
            x = torch.cat((img, scribble_click_embed, mask_input), dim=-3)
    ...
    def embed(self, inputs):
        ...
        # x = torch.cat((image, box_embed, click_embed, mask_input), dim=-3)
        x = torch.cat((image, click_embed, mask_input), dim=-3)
