...
class WarpScribble:
    ...
    def noise_mask(self, shape: Union[Tuple[int],List[int]] = (8,512,512), device = None):
        """
...
class BezierScribble(WarpScribble):
    """
    Generates scribbles by
        1) sampling control points within the mask region
        2) generating Bezier curves using these control points
        3) warping with a random deformation field
        4) then correcting any scribbles that fall outside the mask
        5) optionally, limiting the max area of scribbles to k pixels
    """
    ...
    def _bezier_curve(self, points: np.ndarray, num_points: int = 100) -> np.ndarray:
        """
        ...
        Returns:
            curve_points: (num_points,2) array of points along the curve
        """
        n = len(points) - 1
    ...
    def batch_scribble(self, mask: torch.Tensor, n_scribbles: int = 1) -> torch.Tensor:
        ...
        Args:
            mask: (b,1,H,W) mask in [0,1] to sample scribbles from
            n_scribbles: number of Bezier curves to generate
        Returns:
            scribble_mask: (b,1,H,W) mask(s) of scribbles in [0,1]
        """
        bs = mask.shape[0]

        # Points to sample control points from
        points = torch.nonzero(mask[:, 0, ...])

        def sample_bezier(indices):
            image = np.zeros(mask.shape[-2:] + (1,))

            if len(indices) > 0:
                # Sample control points for each curve
                for _ in range(n_scribbles):
                    idx = np.random.randint(low=0, high=len(indices), size=self.control_points)
                    control_pts = points[indices, 1:][idx, 0, ...]
                    # Flip order of coordinates to be xy
                    control_pts = torch.flip(control_pts, dims=(1,)).cpu().numpy()
                    # Generate and draw Bezier curve
                    curve_points = self._bezier_curve(control_pts)
        ...
        scribbles = torch.stack([
            sample_bezier(torch.argwhere(points[:, 0] == i)) for i in range(bs)
        ]).to(mask.device).moveaxis(-1, 1).float()  # shape: b x 1 x H x W
        ...
        # Remove curves outside the mask
        corrected_warped_scribbles = mask * warped_scribbles
        ...
        if self.max_pixels is not None:
            noise = torch.stack([
                voxynth.noise.perlin(shape=mask.shape[-2:], smoothing=self.max_pixels_smooth, magnitude=1,
        ...
        if self.show:
            ...
            if self.max_pixels is not None:
                binary_mask = binary_mask.reshape(*mask.shape)
                tensors = [mask, scribbles, warped_scribbles, noise, binary_mask, corrected_warped_scribbles, mask]
                titles = ["Mask", "Bezier Curves", "Warped Curves", 'Smooth Noise', 'Top k Pixels',
                          'Corrected Scribbles', 'Corrected Scribbles']
            else:
                tensors = [mask, scribbles, warped_scribbles, corrected_warped_scribbles, mask]
                titles = ["Mask", "Bezier Curves", "Warped Curves", 'Corrected Scribbles', 'Corrected Scribbles']
