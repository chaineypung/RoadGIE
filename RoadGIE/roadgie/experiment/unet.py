...
class ScribblePromptExperiment(BaseExperiment):
    ...
    def run(self, resume_tag=None):
        ...
        try:
            ...
            for epoch in epoch_iter:
                ...
                raise ValueError("111111111111111111111111111111")
    ...
    def run_step(self, batch_idx, batch, prompt_iter=1, backward=True, augmentation=False):
        ...
        if augmentation:
            with torch.no_grad():
                ...
                ############################################################
                # imgg = torch.clamp(img.cpu(), 0, 1) * 255
                # segg = seg.cpu() * 255
                # fig, axes = plt.subplots(4, 2, figsize=(10, 20))
                # for i in range(4):
                #     axes[i, 0].imshow(imgg[i].permute(1, 2, 0).byte())
                #     axes[i, 0].axis('off')
                #     axes[i, 1].imshow(segg[i].squeeze().byte(), cmap='gray')
                #     axes[i, 1].axis('off')
                # plt.tight_layout()
                # plt.show()
                ############################################################
        ...
        if backward:
            if self.config.get("train.fp16", True) or self.config.get("train.bf16", True):###################
    ...
    def run_iter(self, img, seg, prompt_iter=1):
        ...
        for i in range(prompt_iter):
            ...
            x = prompts.get("x") # shape: B x 7 x H x W
            y = prompts.get("y") # Shape: B x 1 x H x W

            ############################################################
            # xx = torch.clamp(x.cpu(), 0, 1) * 255
            # yy = y.cpu() * 255
            # fig, axes = plt.subplots(4, 5, figsize=(10, 20))
            # for i in range(4):
            #     axes[i, 0].imshow(xx[i, 0:3, :, :].permute(1, 2, 0).byte())
            #     axes[i, 0].axis('off')
            #
            #     axes[i, 1].imshow(xx[i, 3, :, :].squeeze().byte(), cmap='gray')
            #     axes[i, 1].axis('off')
            #
            #     axes[i, 2].imshow(xx[i, 4, :, :].squeeze().byte(), cmap='gray')
            #     axes[i, 2].axis('off')
            #
            #     axes[i, 3].imshow(xx[i, 5, :, :].squeeze().byte(), cmap='gray')
            #     axes[i, 3].axis('off')
            #
            #     axes[i, 4].imshow(yy[i].squeeze().byte(), cmap='gray')
            #     axes[i, 4].axis('off')
            # plt.tight_layout()
            # plt.show()
            ############################################################
            ...
            if self.config.get("train.fp16", True):################################
    ...
    def compute_metrics(self, outputs):
        ...
        outputs_prob = outputs["seg_pred"].sigmoid()
        outputs_prob[outputs_prob < 0.3] = 0
        outputs_prob[outputs_prob >= 0.3] = 255
        outputs_true = outputs["seg_true"]
        outputs_true[outputs_true == 0] = 0
        outputs_true[outputs_true == 1] = 255

        base_path = "/mnt/data1/pcx/hms/ScribblePrompt/demos"
        images_per_folder = 8
        total_images = outputs_prob.size(0)
        ...
        for i in range(num_folders):
            ...
            folder_path = os.path.join(base_path, str(folder_index), "img")
            os.makedirs(folder_path, exist_ok=True)

            for j in range(start_idx, end_idx):
                img_tensor = outputs_prob[j]
                save_path = os.path.join(folder_path, f"{b}_{j:03d}.png")
        ...
        for i in range(num_folders):
            ...
            folder_path = os.path.join(base_path, str(folder_index), "mask")
            os.makedirs(folder_path, exist_ok=True)

            for j in range(start_idx, end_idx):
                img_tensor = outputs_true[j]
                save_path = os.path.join(folder_path, f"{b}_{j:03d}.png")
                save_image(img_tensor, save_path)
...
if __name__ == "__main__":
    ...
    config_dir = "/mnt/data1/pcx/hms/ScribblePrompt/configs"
    inf_config_dir = "/mnt/data1/pcx/hms/ScribblePrompt/configs"

    parser = argparse.ArgumentParser()
    parser.add_argument("--config", "-config",
        # default=str(config_dir / "train_unet.yaml"),
        default="/mnt/data1/pcx/hms/ScribblePrompt/configs/train_unet.yaml",
