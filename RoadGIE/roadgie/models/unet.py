...
def rescale_inputs(inputs: Dict[str,any], input_size: Tuple[int] = (512,512)):
    """
    Rescale the inputs 
    """ 
...
def prepare_inputs(inputs: Dict[str,torch.Tensor], device = None) -> torch.Tensor:
    ...
    # x = torch.cat((image, box_embed, scribble_click_embed, mask_input), dim=-3)
...
def click_onehot(point_coords, point_labels, shape: Tuple[int,int] = (512,512), indexing: Literal['xy','uv'] ='xy'):
    """
...
def bbox_shaded(boxes, shape: Tuple[int,int] = (512,512), device='cpu'):
    """
